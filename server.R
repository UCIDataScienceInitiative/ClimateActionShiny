...
google_trend <- read.csv(paste0(root.dir, "google_trend.csv"))
google_trend2 <- read.csv(paste0(root.dir, "google_trend2.csv"))
google_trend3 <- read.csv(paste0(root.dir, "pdsidate-2.csv"))
...
###### Water Use
water_use <- read.csv(paste0(root.dir, "drought_water_use.csv"))
  ...
  trend <- input$google_trend
    ...
    output$google.plot <- renderPlot({
      ...
      ggplot(gtmelt,aes_string(x="start_of_week",y="value"))+
        ...
        geom_vline(xintercept = as.numeric(Emergency),size=2,linetype=4)+
        labs(x= "Time", y = "Search Frequency")+
